...
export const SaleViewPage = () => {
  ...
  return (
    ...
    >
      ...
      <Typography variant="p">Método de pago: {data?.paymentMethod}</Typography>
      <Typography variant="p">Productos:</Typography>
      {data?.productsOnSale &&
        data.productsOnSale.map((product) => (
          <div key={product.id} className="flex justify-between">
            ...
            <Typography variant="p">Cantidad: {product.quantity}</Typography>
            <img className="w-10 h-10 mt-4" src={product.image} />
          </div>
        ))}

      <Typography variant="p" className="text-end">
        Total de la venta: ${data?.amount}
      </Typography>
    </Card>
  );
};
